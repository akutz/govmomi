...
// ThreadSafeStore is an interface that allows concurrent indexed access to a
// storage backend.  It is like Indexer but does not (necessarily) know how to
// extract the Store key from a given object.
//
// TL;DR caveats: you must not modify anything returned by Get or List as it
// will break the indexing feature in addition to not being thread safe.
//
// The guarantees of thread safety provided by List/Get are only valid if the
// caller treats returned items as read-only. For example, a pointer inserted in
// the store through `Add` will be returned as is by `Get`. Multiple clients
// might invoke `Get` on the same key and modify the pointer in a
// non-thread-safe way. Also note that modifying objects stored by the indexers
// (if any) will *not* automatically lead to a re-index. So it's not a good idea
...
type ThreadSafeStore interface {
	...
	// indexes after the store already has items.
...
func (i *storeIndex) getKeysFromIndex(
	indexName string, obj any) (sets.Set[string], error) {
...
func (i *storeIndex) getKeysByIndex(
	indexName, indexedValue string) (sets.Set[string], error) {
...
const unableToCalcIndexEntryFmt = "unable to calculate an index entry for key %q on index %q: %v"

// updateSingleIndex modifies the objects location in the named index:
// - for create you must provide only the newObj
// - for update you must provide both the oldObj and the newObj
// - for delete you must provide only the oldObj
// updateSingleIndex must be called from a function that already has a lock on
// the cache
func (i *storeIndex) updateSingleIndex(name string, oldObj any, newObj any, key string) {
	var oldIndexValues, indexValues []string
	indexFunc, ok := i.indexers[name]
	if !ok {
		// Should never happen. Caller is responsible for ensuring this exists,
		// and should call with lock held to avoid any races.
	...
	if oldObj != nil {
		...
		if err != nil {
			panic(fmt.Errorf(unableToCalcIndexEntryFmt, key, name, err))
		}
	} else {
	...
	if len(indexValues) == 1 &&
		len(oldIndexValues) == 1 &&
		indexValues[0] == oldIndexValues[0] {
		// We optimize for the most common case where indexFunc returns a
		// single value which has not been changed
		return
	}
...
// - for update you must provide both the oldObj and the newObj
// - for delete you must provide only the oldObj
// updateIndices must be called from a function that already has a lock on the
// cache
...
// function. Index is thread-safe so long as you treat all items as immutable.
...
// ByIndex returns a list of the items whose indexed values in the given index
// include the given indexed value
